...
桌面版已经把登录和采集流程收进一个界面：点击“扫码登录”，在应用内完成抖音扫码；登录成功后 Cookie 和安全凭证会由本地桥接工具自动接入并保存，不需要手工打开 F12。
...
安装包输出在 `release` 目录。开发运行可以使用 `npm start`；开发环境默认调用系统 `python`，也可以通过 `DOUYIN_PYTHON` 指定 Python 路径。
