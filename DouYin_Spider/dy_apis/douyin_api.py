...
class DouyinAPIError(RuntimeError):
    """Raised when Douyin returns an unusable or unexpected API response."""
...
class DouyinAPI:
    ...
    @staticmethod
    def _resolve_work_url(auth, url: str) -> tuple[str, str]:
        """Return a canonical video URL and aweme id for all common share URLs."""
        raw_url = str(url or "").strip()
        if not raw_url:
            raise ValueError("作品 URL 不能为空。")

        def find_id(candidate: str) -> str | None:
            ...
            # Some copied URLs contain the path without a conventional host.
            match = re.search(r"(?:video|aweme_id|modal_id)[=/](\d+)", candidate)
            return match.group(1) if match else None

        aweme_id = find_id(raw_url)
        ...
        if not aweme_id:
            try:
                ...
                response = requests.get(
                    raw_url,
                    headers=headers.get(),
                    cookies=getattr(auth, "cookie", None),
                    allow_redirects=True,
                    timeout=20,
                    verify=False,
                )
                response.raise_for_status()
                resolved_url = response.url or raw_url
                aweme_id = find_id(resolved_url) or find_id(response.text[:200000])
            except requests.RequestException as exc:
                raise DouyinAPIError(f"无法解析作品链接，短链请求失败: {exc}") from exc

        if not aweme_id:
            raise ValueError("无法从作品链接中识别 aweme_id，请提供有效的抖音作品链接。")
        return f"{DouyinAPI.douyin_url}/video/{aweme_id}", aweme_id

    @staticmethod
    def _parse_json_response(response: requests.Response, api: str) -> dict:
        """Parse an API response with actionable diagnostics instead of bare JSON errors."""
        status = response.status_code
        content_type = response.headers.get("content-type", "")
        body = response.text.lstrip("\ufeff \r\n\t")
        if status >= 400:
            raise DouyinAPIError(
                f"抖音接口 {api} 返回 HTTP {status}，content-type={content_type or 'unknown'}。"
            )
        if not body:
            raise DouyinAPIError(
                f"抖音接口 {api} 返回空响应（HTTP {status}，content-type={content_type or 'unknown'}），"
                "可能触发风控或登录状态已失效。"
            )
        try:
            ...
            hint = "验证/风控页面" if "<html" in body[:500].lower() else "非 JSON 接口响应"
            raise DouyinAPIError(
                f"抖音接口 {api} 返回{hint}（HTTP {status}，content-type={content_type or 'unknown'}，"
                f"响应前缀={preview!r}）。请刷新 Cookie 后重试。"
            ) from exc
        if not isinstance(data, dict):
            raise DouyinAPIError(f"抖音接口 {api} 返回 JSON 类型异常: {type(data).__name__}。")
        return data
    ...
    def get_work_info(auth, url: str) -> dict:
        ...
        url, aweme_id = DouyinAPI._resolve_work_url(auth, url)
        headers = HeaderBuilder().build(HeaderType.GET)
        ...
        return DouyinAPI._parse_json_response(resp, api)
