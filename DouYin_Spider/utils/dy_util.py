...
def generate_webid(auth=None, url=""):
    ...
    try:
        ...
        response = requests.get(url, headers=headers.get(), timeout=20, verify=False)
        response.raise_for_status()
        res_text = response.text
        # The page bootstrap changed from escaped JSON to regular JSON in
        # different Douyin builds. Accept both forms before falling back.
        patterns = (
            r'\\"user_unique_id\\":\\"(.*?)\\"',
            r'"user_unique_id"\s*:\s*"([^"]+)"',
        )
