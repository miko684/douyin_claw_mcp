...
def _pick_work_cover(data):
    """优先使用原始封面，避免把视频首帧当成封面。"""
    video = data.get('video') or {}
    for field in ('origin_cover', 'cover', 'dynamic_cover'):
        cover = _first_media_url(video.get(field))
        if cover:
            return cover
    images = data.get('images') or []
    return _first_media_url(images[0]) if images else ''
...
def handle_work_info(data):
    ...
    video_cover = _pick_work_cover(data)
    title = data['desc']
    desc = data['desc']
